...
export type Translator = (
  key: string,
  replace?: Record<string, string | number | boolean>,
) => string;
...
export const trans: Translator = (key, replace) => {
  return makeReplacements(get(key), replace);
};

export const transChoice: TranslatorChoice = (key, number, replace) => {
...
export const TranslatorPlugin: Plugin = {
  install(app) {
    ...
    app.config.globalProperties.$locales = __i18n_available;
    app.config.globalProperties.$fallbackLocale = __i18n_fallback;
    app.config.globalProperties.$currentLocale = __i18n_current;
  },
};

function get(key: string) {
  let translation = __i18n_resources[key];
